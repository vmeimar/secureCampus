<!DOCTYPE html>
<html style="width: 1200px;">
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, shrink-to-fit=no">
    <title>ΠΡΩΤΟΚΟΛΛΟ ΤΡΙΜΕΛΟΥΣ ΕΠΙΤΡΟΠΗΣ</title>
    <!-- Latest compiled and minified CSS -->
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.5.0/css/bootstrap.min.css">
    <!-- jQuery library -->
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>
    <!-- Popper JS -->
    <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.16.0/umd/popper.min.js"></script>
    <!-- Latest compiled JavaScript -->
    <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.5.0/js/bootstrap.min.js"></script>
</head>
<style>
    body, title, .row {
        font-family: DejaVu Sans, sans-serif;
        font-size: 12px;
    }
</style>
<body>
<header>
    <div><img src="data:image/png;base64,"
              width="240px"></div>
</header>
<div class="container">
    <div>
        <p class="text-center"><strong>ΠΡΩΤΟΚΟΛΛΟ ΤΡΙΜΕΛΟΥΣ ΕΠΙΤΡΟΠΗΣ ΥΠΗΡΕΣΙΩΝ ΦΥΛΑΞΗΣ</strong></p>
    </div>
    <div>
        <p class="text-center">{{ date('d/m/Y') }}</p>
    </div>
    <div>
        <p class="text-left">Οι κάτωθι υπογεγραμμένοι:</p>
        <ol>
            @foreach($committeeMembers as $member)
                <li>{{ $member->surname }} {{ $member->name }}, {{$member->tier}}</li>
            @endforeach
        </ol>
    </div>
    <p>οι οποίοι αποτελούμε την Κεντρική Επιτροπή Παραλαβής υπηρεσιών φύλαξης των κτηρίων του ΕΚΠΑ σύμφωνα με την
        {{ $contract->dean_act }} Πράξη Πρύτανη (ΑΔΑ: {{ $contract->ada }}), αφού λάβαμε υπόψη:<br></p>
    <ul>
        <li>τις βεβαιώσεις των Εποπτών των κτηρίων του ΕΚΠΑ για το διάστημα από {{ $duration['from'] }} μέχρι {{ $duration['to'] }}<br></li>
        <li>την επικοινωνία με τους Επόπτες για την καταγραφή τυχόν παρατηρήσεών τους<br></li>
        <li>τους όρους της από {{ $contract->contract_start_date }} σύμβασης (ΑΔΑΜ : {{ $contract->adam }})<br></li>
        <li>την υπεύθυνη δήλωση του διευθύνοντα συμβούλου της εταιρείας&nbsp; {{ $company->name }} για την
            τήρηση της υποκείμενης εργατικής νομοθεσίας για τον προαναφερθέν χρονικό διάστημα, όσον αφορά υποχρεώσεις της εταιρείας για την υγεία,
            ασφάλεια και ασφάλιση του προσωπικού της εταιρείας που απασχολείται στην φύλαξη του ΕΚΠΑ, καθώς και την καταβολή
            όλων των νόμιμων αποδοχών στους παραπάνω εργαζόμενους (ΕΠΙΣΥΝΑΠΤΕΤΑΙ)<br></li>
        <li>τα δικαιολογητικά που κατέθεσε η Εταιρεία<br></li>
    </ul>
    <p class="text-center"><strong>βεβαιώνουμε</strong></p>
    <p>ότι οι υπηρεσίες φύλαξης στα κτήρια του ΕΚΠΑ για το χρονικό διάστημα από {{ $duration['from'] }} έως {{ $duration['to'] }} έχουν παρασχεθεί σύμφωνα με τις
        προβλεπόμενες ώρες της από {{ $contract->contract_start_date }} σύμβασης εκτός από εκείνες που αναφέρονται στο Παράρτημα A του παρόντος και αποτελεί
        αναπόσπαστο τμήμα αυτού όπως και τo Παράρτημα Β.<br></p>
{{--    <p>Επιπλέον, από τον έλεγχο των βεβαιώσεων των εποπτών διαπιστώθηκε ότι απασχολήθηκαν φύλακες υπερωριακά (Παράρτημα Β).<br></p>--}}
    <p>Επιπλέον, εφόσον διαπιστωθεί από τον έλεγχο των βεβαιώσεων των εποπτών υπερωριακή απασχόληση φυλάκων, θα αποτυπωθεί στο Παράρτημα Β.<br></p>
    <footer>
        <div>
            <p class="text-center">Η ΕΠΙΤΡΟΠΗ</p>
        </div>
        <div>
            <div class="row">
                <div class="col-md-4">
                    <p class="text-left">{{ $committeeMembers[0]->surname }} {{ $committeeMembers[0]->name }}</p>
                </div>
                <div class="col-md-4">
                    <p class="text-center">{{ $committeeMembers[1]->surname }} {{ $committeeMembers[1]->name }}</p>
                </div>
                <div class="col-md-4">
                    <p class="text-right">{{ $committeeMembers[2]->surname }} {{ $committeeMembers[2]->name }}</p>
                </div>
            </div>
        </div>
    </footer>
</div>
</body>
</html>
