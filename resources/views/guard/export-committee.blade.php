<!DOCTYPE html>
<html style="width: 1200px;">
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, shrink-to-fit=no">
    <title>ΠΡΩΤΟΚΟΛΛΟ ΤΡΙΜΕΛΟΥΣ ΕΠΙΤΡΟΠΗΣ</title>
    <!-- Latest compiled and minified CSS -->
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.5.0/css/bootstrap.min.css">

    <!-- jQuery library -->
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>

    <!-- Popper JS -->
    <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.16.0/umd/popper.min.js"></script>

    <!-- Latest compiled JavaScript -->
    <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.5.0/js/bootstrap.min.js"></script>
</head>
<style>
    body, title, .row {
        font-family: DejaVu Sans, sans-serif;
        font-size: 12px;
    }
</style>
<body>
<header>
    <div><img src="data:image/png;base64,"
              width="240px"></div>
</header>
<div class="container">
    <div>
        <p class="text-center"><strong>ΠΡΩΤΟΚΟΛΛΟ ΤΡΙΜΕΛΟΥΣ ΕΠΙΤΡΟΠΗΣ</strong></p>
    </div>
    <div>
        <p class="text-left">Οι κάτωθι υπογεγραμμένοι:</p>
        <ol>
            <li>Βαλάκος Ευστράτιος, Μέλος ΔΕΠ Τμήματος Βιολογίας</li>
            <li>Νοτάρης Σωτήριος, Μέλος ΔΕΠ Τμήματος Μαθηματικού</li>
            <li>Μπάσιος Κων/νος, Διοικητικός Υπάλληλος ΕΚΠΑ</li>
        </ol>
    </div>
    <p>οι οποίοι αποτελούμε την Κεντρική Επιτροπή Παραλαβής υπηρεσιών φύλαξης των κτηρίων του ΕΚΠΑ σύμφωνα με την
        123η / 2018-2019 Πράξη Πρύτανη (ΑΔΑ: ΨΠΞΦ46ΨΖ2Ν-2ΝΟ), αφού λάβαμε υπόψη:<br></p>
    <ul>
        <li>τις βεβαιώσεις των Εποπτών των κτηρίων του ΕΚΠΑ για το διάστημα από {{ $from }} μέχρι {{ $to }}<br></li>
        <li>την επικοινωνία με τους Επόπτες για την καταγραφή τυχόν παρατηρήσεών τους<br></li>
        <li>τους όρους της από 23.04.2019 σύμβασης (ΑΔΑΜ : 19SYMV004853037)<br></li>
        <li>την υπεύθυνη δήλωση του διευθύνοντα συμβούλου της εταιρείας&nbsp; MY SERVICES SECURITY AND FACILITY Α.Ε. για την
            τήρηση της υποκείμενης εργατικής νομοθεσίας για τον προαναφερθέν χρονικό διάστημα, όσον αφορά υποχρεώσεις της εταιρείας για την υγεία,
            ασφάλεια και ασφάλιση του προσωπικού της εταιρείας που απασχολείται στην φύλαξη του ΕΚΠΑ, καθώς και την καταβολή
            όλων των νόμιμων αποδοχών στους παραπάνω εργαζόμενους (ΕΠΙΣΥΝΑΠΤΕΤΑΙ)<br></li>
        <li>τα δικαιολογητικά που κατέθεσε η Εταιρεία<br></li>
    </ul>
    <p class="text-center"><strong>βεβαιώνουμε</strong></p>
    <p>ότι οι υπηρεσίες φύλαξης στα κτήρια του ΕΚΠΑ για το χρονικό διάστημα από {{ $from }} έως {{ $to }} έχουν παρασχεθεί σύμφωνα με τις
        προβλεπόμενες ώρες της από 23.04.2019 σύμβασης εκτός από εκείνες που αναφέρονται στο Παράρτημα A του παρόντος και αποτελεί
        αναπόσπαστο τμήμα αυτού όπως και τo Παράρτημα Β.<br></p>
    <p>Επιπλέον, από τον έλεγχο των βεβαιώσεων των εποπτών διαπιστώθηκε ότι απασχολήθηκαν φύλακες υπερωριακά (Παράρτημα Β).<br></p>
    <footer>
        <div>
            <p class="text-center">Η ΕΠΙΤΡΟΠΗ</p>
        </div>
        <div>
            <div class="row">
                <div class="col-md-4">
                    <p class="text-left">Ε. Δ. Βαλάκος</p>
                </div>
                <div class="col-md-4">
                    <p class="text-center">Σ. Νοτάρης</p>
                </div>
                <div class="col-md-4">
                    <p class="text-right">Κ. Μπάσιος</p>
                </div>
            </div>
        </div>
    </footer>
</div>
</body>
</html>
